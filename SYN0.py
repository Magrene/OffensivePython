...
class TCPPacket:
 def __init__(self, dport = 80, sport = 65535, dst='10.3.3.2', src='192.168.1.101', data = 'Nothing'):
  self.dport = dport
  self.sport = sport
  self.src_ip = src
  self.dst_ip = dst
 ...
 def reassemble_tcp_feilds(self):
  self.raw = struct.pack('!HHLLBBH', self.tcp_src, self.tcp_dst, self.tcp_seq, self.tcp_ack_seq, self.tcp_hdr_len, self.tcp_flags , self.tcp_wdw )+struct.pack("H",self.tcp_chksum)+struct.pack('!H', self.tcp_urg_ptr)
  return
